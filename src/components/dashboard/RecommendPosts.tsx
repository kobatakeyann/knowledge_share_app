  ...
  return (
    <div className="space-y-2">
      ...
      {recommended.map((post) => {
        ...
        const matchingTags = post.tags.filter((t) => currentUser.skills.includes(t));
        return (
          <Link
            key={post.id}
            to="/feed"
            ...
            <div className="flex items-start gap-2">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-1">
                  <span className={`text-xs font-medium ${typeColors[post.type]}`}>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <div className="flex gap-1">
                    {matchingTags.map((tag) => (
                      <span key={tag} className="text-[10px] text-primary-400 bg-primary-500/10 px-1.5 py-0.5 rounded-full">
                        #{tag}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex-shrink-0 text-center">
                <div className="text-sm font-bold text-gray-300">{post.reactions.length}</div>
